...
from statstuff import regression as lir


class Tests(TestCase):

    def test_single_random_event(self):
        self.assertEqual(None, lir.linear([], []))
        self.assertEqual(None, lir.linear([1, 2], []))
        self.assertEqual(None, lir.linear([], [3, 4]))
        self.assertEqual(None, lir.linear([1, 2], [3]))
